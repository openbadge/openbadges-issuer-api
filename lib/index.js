...
var OpenBadges = function (config) {
    ...
    github.getRepoContent(config.user, config.repo, '')
        .then(function (res) {
            ...
            github.getClassesCommits(config.user, config.repo)
                .then(function (res) {
                    var createdClasses = {};
...
OpenBadges.prototype.createClass = function (data) {
    ...
    github.pushFile(config.user, config.repo, path.join(badgeClass, 'class.json'), _class.commitMsg, _class.data);
    github.pushFile(config.user, config.repo, path.join(badgeClass, 'img.png'), img.commitMsg, img.data);
...
OpenBadges.prototype.createBadge = function (data) {
    ...
    github.pushFile(config.user, config.repo, path.join(pathToBadge, id + '.json'), badge.commitMsg, badge.data);
