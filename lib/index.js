...
OpenBadges.prototype.createClass = function (data) {
    /**
     * Checks whether the give class exists
     * @param {Object[]} classes
     * @param {String} className
     * @returns {Boolean}
     */
    function existsClass(classes, className) {
        for (var item in classes) {
            if (classes[item].name === className) {
                return true;
            }
        }
    ...
    /* CLASS CREATION GOES NEXT */

    var config = this.config,
        classes = this.info.classes,
        className = data.name.trim().replace(/( )+/g, '_');

    if (existsClass(classes, className)) return vow.resolve('The Badge Class already exists!');
...
OpenBadges.prototype.createBadge = function (data) {
     ...
     * @returns {Boolean}
     */
    function existsId(classes, id) {
        for (var item in classes) {
            if (classes[item].badges.indexOf(id) > -1) return true;
        }
